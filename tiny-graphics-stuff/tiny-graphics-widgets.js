...
const Canvas_Widget = (widgets.Canvas_Widget = class Canvas_Widget {
	...
	constructor(element, initial_scenes, options = {}) {
		...
		const rules = [
			'.canvas-widget { width: 1080px; background: White; margin:auto }',
		...
		if (this.show_explanation) {
			this.embedded_explanation_area = this.element.appendChild(
				document.createElement('div')
			);
			this.embedded_explanation_area.className = 'text-widget';
		}

		const canvas = this.element.appendChild(
			document.createElement('canvas')
		);

		if (this.make_controls) {
			this.embedded_controls_area = this.element.appendChild(
				document.createElement('div')
			);
		...
		if (this.make_code_nav) {
			this.embedded_code_nav_area = this.element.appendChild(
				document.createElement('div')
			);
		...
		if (this.make_editor) {
			this.embedded_editor_area = this.element.appendChild(
				document.createElement('div')
			);
		...
		this.webgl_manager = new tiny.Webgl_Manager(
			canvas,
			color(0, 0, 0, 1)
		);
		...
		const primary_scene = initial_scenes
			? initial_scenes[0]
			: undefined;
		const additional_scenes = initial_scenes
			? initial_scenes.slice(1)
			: [];
		const primary_scene_definiton = primary_scene
			? primary_scene.constructor
			: undefined;
		if (this.show_explanation)
			this.embedded_explanation = new Text_Widget(
				this.embedded_explanation_area,
				this.webgl_manager.scenes,
				this.webgl_manager
			);
		if (this.make_controls)
			this.embedded_controls = new Controls_Widget(
				this.embedded_controls_area,
				this.webgl_manager.scenes
			);
		if (this.make_editor)
			this.embedded_editor = new Editor_Widget(
				this.embedded_editor_area,
				primary_scene_definiton,
				this
			);
		if (this.make_code_nav)
			this.embedded_code_nav = new Code_Widget(
...
const Controls_Widget =
	(widgets.Controls_Widget = class Controls_Widget {
		...
		constructor(element, scenes) {
			...
			const table = element.appendChild(
		...
		make_panels(time) {
			...
			while (open_list.length) {
				...
				// Draw top label bar:
				control_box.appendChild(
					Object.assign(document.createElement('div'), {
						textContent: scene.constructor.name,
						className: 'control-title',
					})
				);

				const control_panel = control_box.appendChild(
					document.createElement('div')
				);
		...
		render(time = 0) {
			...
			for (let panel of this.panels)
				for (let live_string of panel.querySelectorAll(
					'.live_string'
				))
...
const Code_Manager = (widgets.Code_Manager = class Code_Manager {
...
const Code_Widget = (widgets.Code_Widget = class Code_Widget {
	...
	constructor(element, main_scene, additional_scenes, options = {}) {
		...
		const rules = [
			'.code-widget .code-panel { margin:auto; background:white; overflow:auto; font-family:monospace; \
	...
	build_reader(element, main_scene, additional_scenes, definitions) {
		...
		const code_panel = element.appendChild(
			document.createElement('div')
		);
		...
		this.code_display = code_panel.appendChild(
			document.createElement('div')
		);
	...
	build_navigator(
		...
		// (Internal helper function)
		const class_list = element.appendChild(
			document.createElement('table')
		);
		...
		const main_scene_link = content.appendChild(
			document.createElement('a')
		);
		main_scene_link.href = 'javascript:void(0);';
		main_scene_link.addEventListener('click', () =>
			this.display_code(main_scene)
		);
		...
		const index_src_link = second_cell.appendChild(
			document.createElement('a')
		);
		index_src_link.href = 'javascript:void(0);';
		index_src_link.addEventListener('click', () =>
			this.display_code()
		);
		...
		third_row.innerHTML =
			'<td><b>tiny-graphics.js</b><br>(Always the same)</td> \
	...
	format_code(code_string) {
		...
		for (let t of new Code_Manager(code_string).tokens)
			if (
				...
				const link = this.code_display.appendChild(
					document.createElement('a')
				);
				link.href = 'javascript:void(0);';
				link.addEventListener('click', () =>
					this.display_code(
						tiny[t.value] || this.definitions[t.value]
					)
				);
				link.textContent = t.value;
			} else {
				const span = this.code_display.appendChild(
...
const Editor_Widget = (widgets.Editor_Widget = class Editor_Widget {
	constructor(
		element,
		initially_selected_class,
		canvas_widget,
		options = {}
	) {
		let rules = [
			'.editor-widget { margin:auto; background:white; overflow:auto; font-family:monospace; width:1060px; padding:10px; \
		...
		const form = (this.form = element.appendChild(
		...
		const run_button = (this.run_button = form.appendChild(
		...
		const submit = (this.submit = form.appendChild(
			document.createElement('button')
		));
		submit.type = 'submit';
		submit.textContent = 'Save as New Webpage';

		const author_box = (this.author_box = form.appendChild(
		...
		const password_box = (this.password_box = form.appendChild(
			document.createElement('input')
		));
		...
		const overwrite_panel = (this.overwrite_panel = form.appendChild(
			document.createElement('span')
		));
		...
		const submit_result = (this.submit_result = form.appendChild(
			document.createElement('div')
		));
		submit_result.style = 'margin: 10px 0';

		const new_demo_code = (this.new_demo_code = form.appendChild(
	...
	submit_demo() {
		const form_fields = Array.from(this.form.elements).reduce(
			(accum, elem) => {
				if (
					elem.value &&
					!(
		...
		return this.fetch_handler(
			'/submit-demo?Unapproved',
			JSON.stringify(form_fields)
		)
			.then((response) => {
				if (response.show_password)
					this.password_box.style.display = 'inline';
				if (response.show_overwrite)
					this.overwrite_panel.style.display = 'inline';
				this.submit_result.innerHTML += response.message + '<br>';
			})
			.catch((error) => {
				this.submit_result.innerHTML +=
					'Error ' + error + ' when trying to upload.<br>';
			});
	}
});

const Text_Widget = (widgets.Text_Widget = class Text_Widget {
	...
	constructor(element, scenes, webgl_manager) {
		const rules = [
			'.text-widget { background: white; width:1060px;\
	...
	render(time = 0) {
		if (this.scenes[0])
			this.scenes[0].show_explanation(
				this.element,
				this.webgl_manager
			);
		else this.event = window.requestAnimFrame(this.render.bind(this));
	}
});
